...
namespace TxtToCfdi
{
    public class ParserAdo : IParser
    {
        ...
        private List<Comprobante> ParseOriginal(string fileName)
        {
            ...
            if (datos.Length > 0)
            {
                ...
                #region Definicion de cabeceras

                var adendaADO = datos.FirstOrDefault(j => j[0] == "ADO");
                ...
                if (adendaADO != null)
                {
                    ...
                    Addenda addendaAdo = new Addenda();
                    
                    AddendaAddenda addendaAdoAd = new AddendaAddenda();
                    
                    addendaAdoAd.valor = adendaADO[1];

                    AddendaProveedor addendaAdoProv = new AddendaProveedor();

                    addendaAdoProv.tipoAddenda = adendaADO[2];

                    addendaAdo.addenda = addendaAdoAd;
                    addendaAdo.proveedor = addendaAdoProv;

                    comprobante.AddendaAdo = addendaAdo;
                }
                #endregion
        ...
        public List<Comprobante> Parse(string fileName)
        {
            ...
            if (comp != null)
            {
                var adenda = this.GetAdoAdenda(data);
                if (adenda != null)
                {
                    comp.AddendaAdo = adenda;
                    comp.XmlAdenda = AddendaSerializer.GetXmlStringFromAddendaObject(adenda, typeof(Addenda), "cfdi", "http://recepcioncfd.ekomercio.com/ADO");
                }
                res.Add(comp);
            }
            return res;
        ...
        private Addenda GetAdoAdenda(string[][] datosArchivo)
        {
            #region GetData
            var adendaADO = datosArchivo.FirstOrDefault(j => j[0] == "ADO");
            #endregion
            if (adendaADO != null)
            {
